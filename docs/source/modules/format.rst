...
allele_resolution
------------------------------
Truncate allele resolutions to a uniform level. This is specially useful prior to
association analyses. Below are some examples of how alleles are normalized at different
resolutions.
::
...
from_ukb
------------------------------
Converts the imputed HLA data from the UK Biobank to :ref:`alt_format`.
::

    altools format from_ukb --phenotype file.pheno input.alt

Input
    The input file is a tab-separated file containing the imputed HLA data from
    the UK Biobank.
...
Output
    An :ref:`alt_format` file will be generated containing the genotype for each
    sample. The default output name is *output.alt*.

from_kirmapper
------------------------------
Converts the KIR genotyping results from `KIR-mapper`_ to :ref:`alt_format`.
::

    altools format from_kirmapper kir_results.tsv --output output.alt

.. _KIR-mapper: https://github.com/erickcastelli/kir-mapper

Input
    glob pattern to the report files generated by `KIR-mapper`_.
...
--remove_pheno_zero:
    If this flag is set, samples with phenotype 0 will be removed from the output
    file.
...
from_immuanot
------------------------------
Converts the HLA genotyping results from `Immuannot`_ to :ref:`alt_format`.
::

    altools format from_immuanot immuanot_results.tsv --output output.alt

.. _Immuannot: https://github.com/YingZhou001/Immuannot

Input
    The report file generated by `Immuannot`_.
...
Input
    An :ref:`alt_format` file containing HLA alleles.
Output
    An :ref:`alt_format` file with grouped HLA alleles.